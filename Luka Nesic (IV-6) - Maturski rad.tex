...
\def\QEDidx{\index{szto je trebalo dokazati@{\sv}to je trebalo dokazati $(\qed)$}}
\def\th{t_{1/2}}
\def\um#1{\,{\rm#1}}
\newdimen\mm \mm=1truemm
\def\cis{\mathop{\rm cis}}
\def\idxaxis#1{\index{#1 osa@$#1$-osa}}
...
Sama re{\cv} {\sl logaritam\/}\index{logaritam} poti{\cv}e od gr{\cv}kih re{\cv}i 
...
\subsection{Definicija logaritma}\index{definicija}
...
% b=\sqrt[y]x,
$$
gde je $b$ {\sl osnova\/}\index{osnova} ({\sl baza\/})\index{baza} logaritma, a $x$ {\sl\idx{argument}}.
...
Funkcija je u skupu realnih brojeva $\Rset$ definisana za $x>0$ i $b>0\land b\ne1$.
...
Funkcija ima jednu {\sl nulu}, uvek za $x=1$. Kada $x\to 0$, onda $y\to{-\infty}$\index{beskona{\cv}nost $(\infty)$}
za $b>1$, odnosno, $y\to{+\infty}$ za $b<1$.
$$
\slika{\includegraphics[]{log.1}}{Grafik\index{grafik} logaritamske funkcije $y=\logb x$.}
$$
...
\def\queq{\index{kvadratna jedna{\cv}ina}}
...
\begin{center}
\def\QR{\GitHubRaw Luka Nesic (IV-6) - Maturski rad.pdf}
\qrcode{\QR}\index{QR}\\
\QR
\end{center}
